...
export async function courseRoutes(fastify) {
  fastify.get('/', { preHandler: requireActiveSubscription }, async (req, reply) => {
    ...
    const isPrivileged = req.user.role === 'ADMIN' || req.user.role === 'CLIENT'

    const sub = isPrivileged ? null : await fastify.prisma.subscription.findFirst({
      where: { userId: req.user.id, status: { in: ['ACTIVE', 'TRIAL'] } },
      orderBy: { createdAt: 'desc' }
    })

    const isPaid = isPrivileged || sub?.planType === '30_DAYS' || sub?.planType === 'VITALICIO'
